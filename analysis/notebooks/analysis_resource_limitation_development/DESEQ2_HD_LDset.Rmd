...
```{r}
#rlog_ddsLTR1 = assay(rlog(ddsLTR1))[row.names(resLTR105),]
resDEG<-degPatterns(rlog_ddsLTR1, metadata = colDataLTR1,time = "devstage",col="condition", consensusCluster = TRUE)
resDEG
pattLTR3abd <- degPatterns(ma, metadata = colDataLTR3,time = "devstage",col="condition",consensusCluster = TRUE)
pattLTR3abd
```
```{r}
resDEGGD4<-degPatterns(rlog_ddsLTR1, metadata = colDataLTR1,time = "devstage",col="condition", consensusCluster = FALSE) #, groupDifference = 2)
resDEGGD4
```
...
```{r}
#pattLTR3abd
pattLTR3heddata <- resDEG[["plot"]][["data"]]

pattLTR3heddata <- pattLTR3heddata[pattLTR3heddata$cluster=="1",]
...
# grouped boxplot
p <- ggplot(pattLTR3heddata, aes(x=devstage, y=value, fill=condition)) + scale_fill_manual(values = c("#AD0000", "#FF9F00"))  + labs(y = "Z-score of gene abundance", x = "Developmental stage") +  scale_x_discrete(labels=devstagelabs) +
  theme(panel.background = element_blank(),
        strip.background = element_blank(),
        strip.text = element_blank(),
        axis.line = element_line(size = 0.2, colour = "black"),
...
p2 <- p + geom_point(data = pattLTR3heddata[pattLTR3heddata$condition == "HDAL",], alpha = 0.3,color="#AD0000") + geom_line(aes(group = genes), alpha = 0.2,color="#AD0000", data = pattLTR3heddata[pattLTR3heddata$condition == "HDAL",])
p3 <- p2 + geom_point(data = pattLTR3heddata[pattLTR3heddata$condition == "LDAL",], alpha = 0.3,color="#FF9F00") + geom_line(aes(group = genes), alpha = 0.15,color="#FF9F00", data = pattLTR3heddata[pattLTR3heddata$condition == "LDAL",]) + geom_boxplot(linewidth=0.3, width=0.3, outlier.alpha = 0.7)
...
colDataLTR3$devstage<-gsub("I","III",as.character(colDataLTR3$devstage))
...
par(mfrow=c(1,1))
# Make a basic volcano plot
with(resLTR3, plot(log2FoldChange, -log10(pvalue), pch=20, main="Volcano plot", xlim=c(-10,10)))

# Add colored points: blue if padj<0.01, red if log2FC>1 and padj<0.05)
with(subset(resLTR3, padj<.05 ), points(log2FoldChange, -log10(pvalue), pch=20, col="blue"))
with(subset(resLTR3, padj<.05 & abs(log2FoldChange)>2), points(log2FoldChange, -log10(pvalue), pch=20, col="red"))
```
...
resLTR305 <- resLTR3[resLTR3$padj <= 0.05, ]
#resJJlrt05<-na.omit(resJJlrt05)
special_genesLTR305<-row.names(resLTR305)
LTR3_genes <- functional_annotation[is.element(functional_annotation$GeneID,c(special_genesLTR305)),]

write.csv(LTR3_genes,"LTR3_genes.csv", row.names = TRUE)
```
...
#par(mfrow=c(2,3))
#for (x in special_genesLTR3) {
#  plotCounts(ddsLTR3, gene=x, intgroup="condition")
#}

functional_annotation[is.element(functional_annotation$GeneID,c(special_genesLTR3)),]
```
```{r}
special_genesLTR3<-c("Vcard_DToL00292","Vcard_DToL00940","Vcard_DToL02031","Vcard_DToL02358","Vcard_DToL02640","Vcard_DToL03154","Vcard_DToL03271","Vcard_DToL03810","Vcard_DToL04336","Vcard_DToL04618","Vcard_DToL05711","Vcard_DToL06590","Vcard_DToL06954","Vcard_DToL07332","Vcard_DToL07510","Vcard_DToL07792","Vcard_DToL08246","Vcard_DToL08588","Vcard_DToL08649","Vcard_DToL09943","Vcard_DToL10415","Vcard_DToL10510","Vcard_DToL10578","Vcard_DToL12180","Vcard_DToL12488","Vcard_DToL12644","Vcard_DToL13266","Vcard_DToL14303","Vcard_DToL14879","Vcard_DToL16322","Vcard_DToL16392","Vcard_DToL16806","Vcard_DToL17843","Vcard_DToL17883","Vcard_DToL18406","Vcard_DToL18354")
...
padj_specialLRT3 <- resLTR305[c(LTR3_genes$GeneID),]
dim(padj_specialLRT3)

#sum<- rbind(LTR3_genes, padj_specialLRT3)

#write.csv(LTR3_genes,"LTR3_genes.csv", row.names = TRUE)

LTR3_genes$padj <- padj_specialLRT3$padj

#dim(LTR3_genes)
#dim(resLTR305)
#length(special_genesLTR305)
...
```{r}
library(DEGreport)
#ma = assay(rlog(ddsLTR3))[row.names(resLTR305)[1:100],]
pattLTR3abd <- degPatterns(ma, metadata = colDataLTR3,time = "devstage",col="condition",consensusCluster = TRUE)
...
```{r}
pattLTR3abd
pattLTR3abddata <- pattLTR3abd[["plot"]][["data"]]
...
```{r}
#degPatterns(ma, metadata = colDataLTR3,time = "devstage",col="condition")
res<-degPatterns(ma, metadata = colDataLTR3,time = "devstage",col="condition")
res
```
...
topGOLRTLI_BP_ab_select <- topGOLRTLI_BP_ab[order(topGOLRTLI_BP_ab$Significant, decreasing = TRUE),][0:5,]

plot_topGoLRTLI_ab_select=ggplot(topGOLRTLI_BP_ab_select, aes(x=reorder(GO_descr, -Significant), y=Significant)) + geom_bar(stat="identity", fill="#512887") + 
  theme(panel.background = element_blank(),
        strip.background = element_blank(),
        strip.text = element_blank(),
        axis.line = element_line(size = 0.2, colour = "black"),
...
topGOLRTLI_BP_he_select <- topGOLRTLI_BP_he[order(topGOLRTLI_BP_he$Significant, decreasing = TRUE),][0:5,]

plot_topGoLRTLI_he_select=ggplot(topGOLRTLI_BP_he_select, aes(x=reorder(GO_descr, -Significant), y=Significant)) + geom_bar(stat="identity", fill="#512887") + 
  theme(panel.background = element_blank(),
        strip.background = element_blank(),
        strip.text = element_blank(),
        axis.line = element_line(size = 0.2, colour = "black"),
...
write.csv(as.data.frame(resAdult_he), 
          file="resAdult_he.csv")
...
resAdult_hedataoutliers <-  resAdult_hedata %>% filter(log2FoldChange > 5 & diffexpressed != "NO" | log2FoldChange < -5 & diffexpressed != "NO" | -log10(padj) > 10) 
...
p <- ggplot(data=resAdult_hedata, aes(x=log2FoldChange, y=-log10(pvalue), col=diffexpressed, label = delabel)) + geom_point() + theme_classic()  + ylim(0, 20) + theme(aspect.ratio = 8/6)+ geom_vline(xintercept=c(-2, 2), col="black", linetype = 2)  + scale_color_manual(values=c("#062A79", "grey", "#CB0815"))

p

#ggsave("resAdult_hedata.pdf")

#ggsave("resAdult_hedata_.pdf", plot = p, width = 15, height = 30,units = c("cm"),)
...
p <- ggplot(data=resAdult_abdata, aes(x=log2FoldChange, y=-log10(pvalue), col=diffexpressed, label = delabel)) + geom_point() + theme_classic()  + ylim(0, 18) + theme(aspect.ratio = 7/6)+ geom_vline(xintercept=c(-2, 2), col="black", linetype = 2)  + scale_color_manual(values=c("#062A79", "grey", "#CB0815"))
